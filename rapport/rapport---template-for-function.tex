...
\affiliation{
\textsuperscript{1}Master ``Ecologie des forets tropicales'', AgroParistech.\\ \hspace{1em} Campus Agronomique, 97310 Kourou, France.
}
...
{\centering \includegraphics[width=0.8\linewidth]{images/NouveauRMarkdown} 
...
\bottomrule
\end{tabu}
\end{table*}\ignorespacesafterend\egroup
...
{\centering \includegraphics[width=0.8\linewidth]{rapport---template-for-function_files/figure-latex/pressure-1} 
...
Si LaTeX ne parvient pas à trouver de solution pour le retour à la ligne, par exemple parce que du code constitue un bloc insécable trop long, ajouter la commande LaTeX \texttt{\textbackslash{}break} à l'emplacement du retour à la ligne.
...
Un bouton de téléchargement est disponible dans la barre de menu du document: il fonctionnera si le document est aussi tricoté au format PDF et si le nom du fichier est renseigné dans le champ ``download'' de l'entête YAML.
...
Le package \emph{rticle} fournit des modèles d'articles (PLOS, PNAS, etc.), mais nécessite l'utilisation de commandes LaTeX et ne permet pas le tricot en HTML.
